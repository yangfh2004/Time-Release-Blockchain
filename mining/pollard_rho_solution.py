...
class PRSolution:
    ...
    def __init__(self, a1, a2, b1, b2, n, pubkey: PublicKey = None):
        ...
        self.b1 = b1
        self.b2 = b2
        # n is the order of the subgroup of the group G for public key, where n = (p - 1) // 2
        self.n = n
        self.pubkey = pubkey
    ...
    def generate_private_key(self):
        ...
        if self.pubkey:
            if not self._key_validate(x):
                # if x is not for this subgroup, it shall be the residue for the other subgroup
                x += self.n
            if self._key_validate(x):
